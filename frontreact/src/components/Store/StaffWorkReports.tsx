...
export function StaffWorkReports() {
  ...
  // 통계 계산
  const totalWorkHours = filteredReports.reduce((sum, r) => sum + r.workHours, 0);
  const totalOvertimeHours = filteredReports.reduce((sum, r) => sum + r.overtimeHours, 0);
  const avgEfficiency = filteredReports.length > 0 
    ? filteredReports.reduce((sum, r) => sum + r.efficiency, 0) / filteredReports.length 
    : 0;

  // 차트 데이터
  const workHoursChartData = filteredReports.map(report => ({
  ...
  const efficiencyChartData = filteredReports.map(report => ({
    name: report.staffName,
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold">{totalWorkHours}시간</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold">{totalOvertimeHours}시간</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-semibold">{Math.round(avgEfficiency)}%</p>
            </div>
          </div>
        </Card>
